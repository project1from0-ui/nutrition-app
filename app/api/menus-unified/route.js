...
export async function GET(request) {
  try {
    ...
    const limit = parseInt(searchParams.get('limit') || '500'); // デフォルトを500に変更
    ...
    // ソート処理を削除（クライアント側で個別目標ベースのスコアリングを行うため）
    // 件数制限のみ適用
    combinedMenus = combinedMenus.slice(0, limit);
    console.log(`[No Server-Side Sorting] Returning ${combinedMenus.length} items without sorting`);
